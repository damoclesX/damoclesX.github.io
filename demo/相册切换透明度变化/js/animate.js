...
function animate(obj, json, optional){
	...
	if(!obj.timer){
		obj.timer=setInterval(function(){
			...
			for(var i in json){
				if(i == 'opacity'){
					obj.style.opacity=fn(obj.n, obj.start[i], obj.dis[i],count)/100;
					obj.style.filter='alpha(opacity:'+fn(obj.n, obj.start[i], obj.dis[i],count)+')';
					
				}else if (i == 'color' || i == 'backgroundColor'){
					obj.style[i]= 'rgb('+parseInt(fn(obj.n, obj.start[i].r, obj.dis[i].r,count))+','+parseInt(fn(obj.n, obj.start[i].g, obj.dis[i].g,count))+','+parseInt(fn(obj.n, obj.start[i].b, obj.dis[i].b,count))+')';
				}else{
					obj.style[i]=fn(obj.n, obj.start[i], obj.dis[i],count)+'px';
				}
			}
